...
LICENSE_BLOB_LINES_C = comment_block(LICENSE_BLOB)

COPYRIGHT_RE = re.compile(r'Copyright \(c\) (\d+)', re.I)


def update_c_license(name, force=False):
    with open(name) as f:
        orig_lines = list(f)
    ...
    if not found:
        if 'Code generated by' in lines[0]:
            lines[1:1] = ['\n'] + LICENSE_BLOB_LINES_C
        else:
            lines[0:0] = LICENSE_BLOB_LINES_C + ['\n']
        changed = True
